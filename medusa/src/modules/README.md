...
## Marketplace Account Module

`src/modules/marketplace-account` stores external seller account links, such as Best Buy Marketplace and Amazon storefront profiles. The service is registered as `marketplaceAccount` in `medusa-config.js`.

The `marketplace_account` table contains display name, platform, description, URL, CTA label, sort order, active status, and optional metadata. Run Medusa migrations after deploying the module so the Admin seller-account page and public store endpoint can read the table.
