...
- Footer agency attribution with a refined Aceternity-style hover link preview and keyboard-visible focus treatment
...
Vercel Speed Insights is rendered only in production builds. The local storefront development command uses Webpack instead of Turbopack because Next's development debug channel can attempt to close the same `WritableStreamDefaultWriter` twice during route transitions, producing a browser-only stream error. Production builds are unaffected.
...
The storefront has an App Router SEO baseline: shared TechHub metadata and Organization/WebSite JSON-LD, route-specific canonical titles, descriptions, keywords, social metadata, product JSON-LD, plus generated `/sitemap.xml` and `/robots.txt`. Public product, collection, buying-guide, and service pages are indexed; account, auth, cart, checkout, order, search, API, proxy, and cookie-preference routes are excluded. Search falls back to an empty product result set when MeiliSearch is unavailable or the configured key is rejected, so invalid search credentials do not crash the route. See `../docs/storefront-seo.md` for the indexing policy.

Storefront CMS reads use `src/lib/data/content.ts`, which calls the Medusa Content CMS plugin's public `/content` routes through the existing Medusa JS SDK instance. Homepage, About, Inspiration, product detail, and `/buying-guides/[slug]` pages consume published CMS items with static fallbacks where appropriate. Keep CMS reads server-side unless a page specifically needs client refresh behavior, and use the `content` cache tag family when invalidating published content. The homepage fallback hero is brand-forward and intentionally minimal: it overlays ecommerce/service positioning, headline, and primary shop CTA on the `techhub-homepage-hero-banner.png` product lineup visual. The site header remains the only homepage logo. The hero uses viewport-aware `svh` sizing so the first-screen banner fits the visible screen while accounting for the fixed mobile header. The homepage includes a featured product preview grid, followed by a dark support section with all carried brands visible at once and Motion-powered blur emphasis that cycles across the brand names, then the all-products preview grid using the same interactive store product cards and live region pricing. Supplied campaign imagery and the CMS publication requirements for the Home, About, Inquiry, and Inspiration refresh are documented in `../docs/storefront-content-refresh-2026-08-10.md`. Business technology inquiries, repair or upgrade requests, and a marketplace section are populated from the Medusa `GET /store/marketplace-accounts` endpoint only when active backend links exist.
...
Apple Pay domain verification serves `public/.well-known/apple-developer-merchantid-domain-association` directly from the storefront root. Keep `.well-known` paths excluded from country-code redirects so Square and Apple receive a 200 response at `/.well-known/apple-developer-merchantid-domain-association`. Google Pay can render even when Apple Pay is hidden; Apple Pay also requires Safari on an Apple Pay-capable Apple device and the exact checkout hostname registered in Square with valid HTTPS and no verification-file redirect. Run `node scripts/check-square-apple-pay-production.mjs` from the repository root after DNS or Square domain changes.
